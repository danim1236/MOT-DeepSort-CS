...
{
    public class HungarianAlgorithmD : IHungarianAlgorithm
    {
        private readonly double[,] _costsMatrix;
        ...
        public HungarianAlgorithmD(double[,] costsMatrix)
        {
            if (costsMatrix == null)
        ...
        public int[] Solve()
        {
            for (int i = 0; i < _height; i++)
            {
                ...
                for (int j = 0; j < _width; j++)
                    minValue = double.Min(minValue, _costsMatrix[i, j]);

                for (int j = 0; j < _width; j++)
                {
                    _costsMatrix[i, j] -= minValue;

                    if (_costsMatrix[i, j] == 0.0 && !_crossedRows[i] && !_crossedColumns[j])
                    {
        ...
        {
            double minValue = CurrentCostMinimum();
        ...
        private double[,] InitializeCostsMatrix(double[,] baseCostsMatrix)
        {
            ...
            if (height > width)
            {
                double[,] costsMatrix = new double[width, height];
            ...
            return (double[,])baseCostsMatrix.Clone();
        }
        ...
        private double CurrentCostMinimum()
        {
            double minValue = double.MaxValue;
