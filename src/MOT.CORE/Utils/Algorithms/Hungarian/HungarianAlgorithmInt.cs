...
namespace MOT.Core.Utils.Algorithms.Hungarian
{
    ...
    {
        private readonly int[,] _costsMatrix;
        ...
        public HungarianAlgorithmInt(int[,] costsMatrix)
        {
            if (costsMatrix == null)
        ...
        public int[] Solve()
        {
            for (int i = 0; i < _height; i++)
            {
                ...
                for (int j = 0; j < _width; j++)
                    minValue = int.Min(minValue, _costsMatrix[i, j]);

                for (int j = 0; j < _width; j++)
                {
                    _costsMatrix[i, j] -= minValue;

                    if (_costsMatrix[i, j] == 0 && !_crossedRows[i] && !_crossedColumns[j])
                    {
        ...
        {
            int minValue = CurrentCostMinimum();
        ...
        private int[,] InitializeCostsMatrix(int[,] baseCostsMatrix)
        {
            ...
            if (height > width)
            {
                int[,] costsMatrix = new int[width, height];
            ...
            return (int[,])baseCostsMatrix.Clone();
        }
        ...
        private int CurrentCostMinimum()
        {
            int minValue = int.MaxValue;
